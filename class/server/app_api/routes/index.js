...
router.get('/recommend/api', function (req, res, next) {
    ...
    // 3. 批量插入数据库表
    // 3.1 数据库查询的语句
    let sqlStr = "INSERT INTO app_recommend(`goods_id`,`goods_name`,`short_name`, `thumb_url`, `hd_thumb_url`, `image_url`, `price`, `normal_price`, `market_price`, `sales_tip`, `hd_url`) VALUES ?";
    // 3.2 执行语句
...
router.get('/api/recommendshoplist', (req, res) => {
    ...
    // 1.1 数据库查询的语句
    let sqlStr = 'SELECT * FROM app_recommend LIMIT ' + (pageNo - 1) * pageSize + ',' + pageSize;
    // console.log(sqlStr);
...
router.post('/api/login_code', (req, res) => {
    ...
    let sqlStr = "SELECT * FROM app_user_info WHERE user_phone = '" + phone + "' LIMIT 1";
    ...
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) {  // 用户已经存在
                ...
                const addSql = "INSERT INTO app_user_info(user_name, user_phone) VALUES (?, ?)";
                ...
                conn.query(addSql, addSqlParams, (error, results, fields) => {
                    ...
                    if (!error) {
                        ...
                        let sqlStr = "SELECT * FROM app_user_info WHERE id = '" + results.insertId + "' LIMIT 1";
                        conn.query(sqlStr, (error, results, fields) => {
                            if (error) {
                                res.json({err_code: 0, message: '请求数据失败'});
                            } else {
...
router.post('/api/login_pwd', (req, res) => {
    ...
    // 3. 查询数据
    let sqlStr = "SELECT * FROM app_user_info WHERE user_name = '" + user_name + "' LIMIT 1";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) {  // 用户已经存在
                ...
                const addSql = "INSERT INTO app_user_info(user_name, user_pwd) VALUES (?, ?)";
...
router.get('/api/user_info', (req, res) => {
    ...
    // 1.1 数据库查询的语句
    let sqlStr = "SELECT * FROM app_user_info WHERE id = '" + userId + "' LIMIT 1";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            res.json({err_code: 0, message: '请求数据失败'});
        } else {
...
router.post('/api/change_user_msg', (req, res) => {
    ...
    // 3. 更新数据
    let sqlStr = "UPDATE app_user_info SET user_name = ? , user_sex = ?, user_address = ?, user_birthday = ?, user_sign = ? WHERE id = " + id;
